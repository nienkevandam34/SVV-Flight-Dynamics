...
time      = data["time"]
stat1_ind = np.where(np.logical_and(time>=1157, time <= 1920)) # index for stationary flight 1
t_stat1   = time[stat1_ind]
aoa       = data["vane_AOA"][stat1_ind]
...
# 6 points only
timestamps = np.array([1157, 1297, 1426, 1564, 1787, 1920])
index_6 = []
for i in range(6):
...
CLadeg, CLa0deg = np.polyfit(aoa, CL, 1) # lift curve gradient and CL at alpha = 0
CLarad, CLa0rad = np.polyfit(np.deg2rad(aoa), CL, 1) # lift curve gradient and CL at alpha = 0
...
e = (CL**2)/(CD-CD0)/(np.pi*(15.911**2/S))
...
Cma    = -1.5        # longitudinal stabilty [ ]          RANDOM GUESS!!!!
Cmde   = -0.6        # elevator effectiveness [ ]         RANDOM GUESS!!!!
